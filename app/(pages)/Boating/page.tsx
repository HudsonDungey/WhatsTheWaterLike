...
const Boating = () => {
  const boatingConditionsPercent = 54;
  const currentDate = new Date().toLocaleDateString();
  const [hideSpots, setHideSpots] = useState(true);
  const [boatingSpots, setBoatingSpots] = useState<FishingSpot[]>([]);
  const router = useRouter();

  const fetchBoatingSpots = async () => {
    const spots = await getFishingSpots();
    if (spots) {
      setBoatingSpots(spots);
    }
  };

  const handleBoatingSpots = () => {
    if (hideSpots) {
      setHideSpots(false);
      fetchBoatingSpots();
    } else {
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-[60px]">
      ...
      <div className="relative w-full h-44 md:h-96">
        <h1 className="absolute top-4 left-[30px] md:left-[60px] text-white text-[40px] md:text-[60px] font-bold z-10">Boating</h1>
        <Image
      ...
      <div className="mx-auto h-[390px] md:h-[600px] px-[30px] md:px-[50px] py-2 flex flex-col md:grid md:grid-cols-12 gap-4 md:gap-8">
        <div className="bg-white col-span-8 shadow-md rounded-lg p-1 mb-[100px]">
          <h2 className="text-xl md:text-3xl pl-6 pt-4 text-start font-semibold text-gray-700">Boating Conditions</h2>
          ...
          <h1 className='text-green-600 md:pl-6 pt-6 mb-2 text-[20px] md:text-[40px] font-bold'>FAIR</h1>
          <div className="flex flex-col items-center justify-center">
            <div className="relative w-[140px] h-[140px] md:w-[200px] md:h-[200px] rounded-full bg-gray-200">
              <div
                ...
                style={{
                  padding: '50px',
                  background: `conic-gradient(#636AE8FF ${boatingConditionsPercent}%, #e5e7eb ${boatingConditionsPercent}%)`,
                }}
              />
            ...
            <div className="flex mt-4">
              {[...Array(5)].map((_, index) => (
                <FaStar
                  key={index}
                  className={`text-4xl ${index < Math.round(boatingConditionsPercent / 20) ? 'text-yellow-400' : 'text-gray-300'}`}
                />
              ))}
            </div>
          </div>
        ...
        <div className="hidden col-span-4 md:flex flex-col">
          <div className="bg-white shadow-md rounded-lg p-4 flex flex-col space-y-4">
            <button
              ...
              onClick={handleBoatingSpots}
            >
              {hideSpots ? "Show Boating Locations" : "Hide Boating Locations"}
            </button>
            <div className="relative h-64 mt-4">
              <MapComponent
                ...
                latitude={-12.4578}
                showSpots={!hideSpots}
                fishingSpots={!hideSpots ? boatingSpots : []}
              />
            </div>
            <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-[40px] p-[20px] md:p-[50px]">
        <div className="bg-purple-100 p-3 rounded-lg text-start shadow-md flex flex-col justify-between">
          ...
          <p className="text-3xl font-semibold text-gray-800">Moderate</p>
        ...
        <div className="bg-green-100 p-3 rounded-lg text-start shadow-md flex flex-col justify-between">
          ...
          <p className="text-3xl font-semibold text-gray-800">6:00 am - 7:00 pm</p>
        </div>
        ...
        <div className="bg-blue-100 p-3 rounded-lg text-start shadow-md flex flex-col justify-between">
          ...
          <div className="flex flex-col items-end text-sm text-gray-500 space-y-1">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="bg-red-500 text-white px-2 py-1 font-semibold rounded-full text-base">2.2m</span>
            </div>
          </div>
        </div>
      </div>
