

export const newSouthWales = [
    "Bondi Beach",
    "Byron Bay",
    "Manly Beach",
    "Coogee Beach",
    "Coffs Harbour",
    "Port Macquarie",
    "Jervis Bay",
    "Narooma",
    "Wollongong",
    "Newcastle",
    "Batemans Bay",
    "Lennox Head",
    "Yamba",
    "Forster",
    "Nelson Bay",
    "Kiama",
    "Cronulla",
    "Palm Beach",
    "Eden",
    "Tweed Heads",
    "Merimbula",
    "Lord Howe Island",
    "Shellharbour",
    "Huskisson",
    "Mollymook",
    "Avoca Beach",
    "Terrigal",
    "Shoalhaven Heads",
    "Fingal Bay",
    "Tathra"
];

export const victoria = [
    "St Kilda",
    "Torquay",
    "Lorne",
    "Apollo Bay",
    "Sorrento",
    "Portsea",
    "Mornington",
    "Geelong",
    "Barwon Heads",
    "Bells Beach",
    "Inverloch",
    "Lakes Entrance",
    "Phillip Island",
    "Wilson's Promontory",
    "Port Campbell",
    "Warrnambool",
    "Anglesea",
    "Queenscliff",
    "Port Fairy",
    "Venus Bay",
    "Rye",
    "Dromana",
    "Cowes",
    "Mallacoota",
    "Cape Bridgewater",
    "Portland",
    "Rosebud",
    "Aireys Inlet",
    "Marlo",
    "Corinella"
];

export const queensland = [
    "Gold Coast",
    "Sunshine Coast",
    "Noosa Heads",
    "Surfers Paradise",
    "Cairns",
    "Port Douglas",
    "Airlie Beach",
    "Townsville",
    "Hervey Bay",
    "Fraser Island",
    "Mooloolaba",
    "Burleigh Heads",
    "Mission Beach",
    "Coolangatta",
    "Caloundra",
    "Hamilton Island",
    "Bribie Island",
    "Yeppoon",
    "Mackay",
    "Tangalooma",
    "Rainbow Beach",
    "Palm Cove",
    "Magnetic Island",
    "Cape Tribulation",
    "Agnes Water",
    "Bundaberg",
    "Cooktown",
    "Bargara",
    "Green Island",
    "Seventeen Seventy"
];


export const southAustralia = [
    "Glenelg",
    "Victor Harbor",
    "Port Lincoln",
    "Kangaroo Island",
    "Robe",
    "Yorke Peninsula",
    "Coffin Bay",
    "Second Valley",
    "Normanville",
    "Port Elliot",
    "Middleton",
    "Goolwa",
    "Vivonne Bay",
    "Streaky Bay",
    "Elliston",
    "Rapid Bay",
    "American River",
    "Whyalla",
    "Ceduna",
    "Moonta Bay",
    "Arno Bay",
    "Kingston SE",
    "Penneshaw",
    "Cape Jervis",
    "Coffin Bay National Park",
    "Wallaroo",
    "Cowell",
    "Port Vincent",
    "Hardwicke Bay",
    "Marion Bay"
];


export const westernAustralia = [
    "Cottesloe Beach",
    "Broome",
    "Rottnest Island",
    "Margaret River",
    "Esperance",
    "Coral Bay",
    "Exmouth",
    "Fremantle",
    "Mandurah",
    "Geraldton",
    "Busselton",
    "Albany",
    "Dunsborough",
    "Denmark",
    "Kalbarri",
    "Shark Bay",
    "Jurien Bay",
    "Augusta",
    "Hamelin Bay",
    "Yallingup",
    "Dongara",
    "Port Hedland",
    "Carnarvon",
    "Lancelin",
    "Leeman",
    "Two Rocks",
    "Eagle Bay",
    "Karratha",
    "Hillarys",
    "Pemberton"
];
  

export const tasmania = [
    "Hobart",
    "Freycinet",
    "Bicheno",
    "Coles Bay",
    "Bruny Island",
    "Port Arthur",
    "Strahan",
    "Burnie",
    "St Helens",
    "Bay of Fires",
    "Devonport",
    "Eaglehawk Neck",
    "Maria Island",
    "Swansea",
    "King Island",
    "Launceston",
    "Cygnet",
    "Orford",
    "Triabunna",
    "Stanley",
    "Wynyard",
    "Richmond",
    "Tasman Peninsula",
    "Boat Harbour Beach",
    "Flinders Island",
    "Dover",
    "Southport",
    "White Beach",
    "Nubeena",
    "Cradle Mountain"
];


export const northernTerritory = [
    "Darwin",
    "Cullen Bay",
    "Casuarina Beach",
    "Mindil Beach",
    "East Point",
    "Nightcliff",
    "Wagait Beach",
    "Cox Peninsula",
    "Nhulunbuy",
    "Maningrida",
    "Melville Island",
    "Bathurst Island",
    "Gove Peninsula",
    "Cobourg Peninsula",
    "Groote Eylandt",
    "Dundee Beach",
    "Tiwi Islands",
    "Wurrumiyanga",
    "Channel Point",
    "Berry Springs",
    "Gunn Point",
    "Yirrkala",
    "Milikapiti",
    "Elcho Island",
    "Ramingining",
    "Milikapiti",
    "Howard Springs",
    "Crocodile Islands",
    "Belyuen"
];


export const actWaterLocations = [
    "Lake Burley Griffin",
    "Cotter River",
    "Molonglo River",
    "Murrumbidgee River",
    "Lake Tuggeranong",
    "Lake Ginninderra",
    "Googong Dam",
    "Uriarra Crossing",
    "Pine Island Reserve",
    "Point Hut Crossing"
];


export const australiaLocations = {
    newSouthWales,
    victoria,
    queensland,
    southAustralia,
    westernAustralia,
    tasmania,
    northernTerritory,
    actWaterLocations
};

export const australianStates = [
    { url: '/northernterritory', text: 'Northern Territory' },
    { url: '/SouthAustralia', text: 'South Australia' },
    { url: '/WesternAustralia', text: 'Western Australia' },
    { url: '/NewSouthWales', text: 'New South Wales' },
    { url: '/Victoria', text: 'Victoria' },
    { url: '/Queensland', text: 'Queensland' },
    { url: '/Tasmania', text: 'Tasmania' },
];


export const australiaLocationsString = [
    "Sydney",          // New South Wales
    "Melbourne",       // Victoria
    "Brisbane",        // Queensland
    "Adelaide",        // South Australia
    "Perth",           // Western Australia
    "Hobart",          // Tasmania
    "Darwin",          // Northern Territory
    "Gold Coast",      // Queensland
    "Newcastle",       // New South Wales
    "Cairns",          // Queensland
    "Townsville",      // Queensland
    "Sunshine Coast",  // Queensland
    "Coffs Harbour",   // New South Wales
    "Byron Bay",       // New South Wales
    "Geraldton",       // Western Australia
    "Broome",          // Western Australia
    "Mandurah",        // Western Australia
    "Mackay",          // Queensland
    "Port Lincoln",    // South Australia
    "Batemans Bay",    // New South Wales
    "Albany",          // Western Australia
    "Devonport",       // Tasmania
    "Port Hedland",    // Western Australia
    "Exmouth",         // Western Australia
    "Airlie Beach",    // Queensland
];

  
  
  
  
  
  
  
  
  